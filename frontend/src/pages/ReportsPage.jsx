...
import { Download, Lock, TrendingUp, TrendingDown, RefreshCw } from 'lucide-react';
import {
...
function KpiMini({ label, value, icon, color }) {
  return (
    <div className="card text-center">
      ...
      <p className={`text-xl font-extrabold ${color}`}>{formatCurrency(value)}</p>
...
function UpgradeModal({ onClose }) {
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center animate-fade-in">
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm mb-6">
          La exportación a CSV está disponible en el plan <strong>Pro</strong> o <strong>Family</strong>.
          Sube de plan para desbloquear exportaciones ilimitadas.
        </p>
        <div className="space-y-2">
          <button className="w-full btn-primary gap-2">
            ⭐ Ver planes — desde S/. 4.99/mes
          </button>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-4">
            <KpiMini label="Ingresos" value={data.income}  icon="📈" color="text-emerald-600 dark:text-emerald-400" />
            <KpiMini label="Egresos"  value={data.expense} icon="📉" color="text-rose-600    dark:text-rose-400"    />
            <KpiMini label="Neto"     value={data.net}     icon="⚖️"
              color={data.net >= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-rose-600 dark:text-rose-400'} />
          </div>
                  ...
                  <div key={r.categoryId || r.type} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-sm font-bold text-gray-800 dark:text-white ml-2 flex-shrink-0">
    ...
    <div className="space-y-5">
      ...
      {loading ? (
        ...
        <>
          {/* KPIs */}
          <div className="grid grid-cols-3 gap-4">
            <KpiMini label="Ingresos totales" value={totalIncome}  icon="📈" color="text-emerald-600 dark:text-emerald-400" />
            <KpiMini label="Egresos totales"  value={totalExpense} icon="📉" color="text-rose-600    dark:text-rose-400"    />
            <KpiMini label="Neto acumulado"   value={totalNet}     icon="⚖️"
              color={totalNet >= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-rose-600 dark:text-rose-400'} />
          </div>

          {/* Bar chart */}
          <div className="card">
            <h3 className="font-bold text-gray-800 dark:text-white mb-4 flex items-center gap-2">
              <span>📊</span> Ingresos vs Egresos — últimos 12 meses
            </h3>
            ...
            {bestMonth && (
              <div className="card bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 uppercase tracking-wide">Mejor mes</span>
                </div>
                <p className="font-bold text-gray-800 dark:text-white">{fmtEvLabel(bestMonth.label)}</p>
                <p className="text-emerald-600 dark:text-emerald-400 font-extrabold text-lg">{formatCurrency(bestMonth.net)}</p>
              </div>
            )}
            {worstMonth && (
              <div className="card bg-rose-50 dark:bg-rose-900/20 border border-rose-100 dark:border-rose-800">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs font-semibold text-rose-600 dark:text-rose-400 uppercase tracking-wide">Peor mes</span>
                </div>
                <p className="font-bold text-gray-800 dark:text-white">{fmtEvLabel(worstMonth.label)}</p>
                <p className="text-rose-600 dark:text-rose-400 font-extrabold text-lg">{formatCurrency(worstMonth.net)}</p>
              </div>
            )}
          </div>
        </>
        ...
        <>
                ...
                <div className="flex-1 w-full space-y-2">
                  {chartData.map((d) => (
                    <div key={d.name} className="flex items-center gap-2 text-sm">
                      ...
                      <span className="font-semibold text-gray-800 dark:text-white">{formatCurrency(d.value)}</span>
          ...
          <div className="card">
            <h3 className="font-bold text-gray-800 dark:text-white mb-4 flex items-center gap-2">
              ...
              <span className="ml-auto text-sm font-normal text-gray-400">Total: {formatCurrency(totalAmt)}</span>
            </h3>
            <div className="space-y-3">
              {filtered.map((r, i) => (
                <div key={r.categoryId || i} className="flex items-center gap-3">
                  <span className="text-xs font-bold text-gray-300 dark:text-gray-600 w-5 text-center">
                    {i + 1}
                  </span>
                  <div className="w-8 h-8 rounded-xl flex items-center justify-center text-base flex-shrink-0"
                    style={{ backgroundColor: (r.category?.color || '#6366f1') + '22' }}>
...
export default function ReportsPage() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (err.response?.status === 403) setShowUpgrade(true);
    } finally {
      setCsvLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      {/* Tabs + Export en la misma fila */}
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">

        {/* Tab selector */}
        <div className="flex gap-1 p-1 bg-gray-100 dark:bg-gray-800 rounded-2xl w-fit flex-shrink-0">
          {TABS.map((t) => (
        ...
        {/* Export CSV — compacto, pegado a la derecha en desktop */}
        <div className="sm:ml-auto flex items-center gap-2 flex-wrap">
          <input type="date" className="input-base text-xs h-9 px-2 w-36"
          ...
          <button onClick={handleExport} disabled={csvLoading}
            className="btn-primary gap-1.5 h-9 text-xs px-3 flex-shrink-0 disabled:opacity-50">
            {csvLoading ? <span className="animate-spin text-sm">⏳</span> : <Download className="w-3.5 h-3.5" />}
            CSV
            <Lock className="w-3 h-3 opacity-60" />
          </button>
          <button onClick={handleExportPdf} disabled={pdfLoading}
            className="flex items-center gap-1.5 h-9 text-xs px-3 flex-shrink-0 font-semibold rounded-xl border border-rose-200 dark:border-rose-800 text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-900/20 transition disabled:opacity-50">
            {pdfLoading ? <span className="animate-spin text-sm">⏳</span> : <Download className="w-3.5 h-3.5" />}
            PDF
            <Lock className="w-3 h-3 opacity-60" />
          </button>
        </div>
